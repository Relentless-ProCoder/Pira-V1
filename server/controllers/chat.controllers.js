...
const newGroupChat = TryCatch(async (req, res, next) => {
  ...
  if (members.length < 2) {
    return next(ErrorHandler("Group must have at least 3 members", 404));
  }
...
const addMembers = TryCatch(async (req, res, next) => {
  ...
  if (!chat) {
    next(ErrorHandler("Chat not found", 404));
  }

  if (!chat.groupChat)
    return next(ErrorHandler("This is not a group chat", 400));
  ...
  if (chat.members.length > 100)
    return next(
      ErrorHandler("you can not able add more than 100 members", 400)
    );
  ...
  emitEvent(
    ...
    ALERT,
    chat.members,
    `${allUserName} has been added in the ${chat.name} group`
  );
...
const leaveGroup = TryCatch(async (req, res, next) => {
  ...
  if (!chat) next(ErrorHandler("Chat not found", 404));

  if (!chat.groupChat) next(ErrorHandler("this is not a group chat", 404));
...
const sendAttachment = TryCatch(async (req, res, next) => {
  ...
  if (files.length < 1)
    return next(ErrorHandler("Please Upload Attachments", 400));

  if (files.length > 5)
    return next(ErrorHandler("Files Can't be more than 5", 400));

  const [chat, me] = await Promise.all([
    Chat.findById(chatId),
    User.findById(req.user, "name"),
  ]);
  if (!chat) return next(ErrorHandler("chat is not found", 400));
...
const renameGroup = TryCatch(async (req, res, next) => {
  ...
  if (chat.creator.toString() !== req.user.toString())
    return next(ErrorHandler("You are not allowed to rename the group", 403));
...
const deleteChat = TryCatch(async (req, res, next) => {
  ...
  if (chat.groupChat && chat.creator.toString() !== req.user.toString()) {
    return next(ErrorHandler("you are not allow to delete chat", 404));
  }
  if (!chat.groupChat && !chat.members.includes(req.user.toString())) {
    return next(ErrorHandler("You are not allowed to delete the chat", 403));
  }

  const members = chat.members;

  // now message with attachments
